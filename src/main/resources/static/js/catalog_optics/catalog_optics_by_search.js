...
function updateURLWithFilters() {
  ...
  const groupNameFromURL = pathMatch ? decodeURIComponent(pathMatch[1]) : "";
  const baseUrl = groupNameFromURL
    ? window.location.origin + `/search/` + groupNameFromURL
...
function updateGroupDisplay() {
  let displayText = "Сонцезахисні окуляри та оправи";
  const filters = [];
...
function activateCheckboxesFromURL() {
  ...
  const filtersSegment = urlPath.split("/search/")[1]?.split("/").pop() || "";
...
function sortOptics(sortCriteria) {
  ...
  let filtersPath = baseUrl.split("/search/")[1] || "";
  let newUrl = window.location.origin + `/search/` + filtersPath;
