...
function updateURLWithFilters() {
  const baseUrl = formattedGroupName
    ? window.location.origin +
      `/catalog/` +
      transliterateUkrainianToEnglish(formattedGroupName)
    : window.location.origin;
  const filters = [];
  Object.keys(activeFilters).forEach((filterType) => {
...
function updateGroupDisplay() {
  let displayText = groupJS + groupGenderJS;
  const filters = [];
...
function activateCheckboxesFromURL() {
  ...
  const filtersSegment = urlPath.split("/catalog/")[1]?.split("/").pop() || "";
...
function sortOptics(sortCriteria) {
  ...
  let filtersPath = baseUrl.split("/catalog/")[1] || "";
  let newUrl = window.location.origin + `/catalog/` + filtersPath;
