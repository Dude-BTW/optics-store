 ...
 */
const opticsByCategoryContainer = document.getElementById(
  "all-optics-byCategory",
);
const opticsByCategory = Array.from(
  opticsByCategoryContainer.querySelectorAll(".card-byCategory"),
);
let filteredOptics = [...opticsByCategory];
let itemsPerPage = 28;
let currentPage = 1;
...
function renderCurrentPage() {
  ...
  opticsByCategoryContainer.innerHTML = "";

  filteredOptics.slice(start, end).forEach((optic) => {
    opticsByCategoryContainer.appendChild(optic);
  });
...
function initPagination() {
  filteredOptics = [...opticsByCategory];
...
document.addEventListener("DOMContentLoaded", () => {
  const cards = Array.from(document.querySelectorAll(".card-byCategory"));
  ...
  const filters = document.querySelectorAll(".drop-down-filter1.show");
...
function toggleCheckbox(element) {
  ...
  const filterType = element
    .closest(".drop-down-filter1")
    .id.replace("filter-", "");
  const filterValue = element.getAttribute("data-originalByBrand-option");
...
function updateURLWithFilters() {
  const baseUrl = formattedCategoryName
    ? window.location.origin + `/brand/` + formattedCategoryName
...
function filterOptics() {
  filteredOptics = opticsByCategory.filter((optic) => {
...
function updateCategoryDisplay() {
  const baseCategoryName = "Сонцезахисні окуляри та оправи";
  let displayText = baseCategoryName + " " + categoryJS;
  const filters = [];
  Object.keys(activeFilters).forEach((filterType) => {
    if (filterType === "group") {
      return;
  ...
  categoryDisplayElement.textContent = displayText;
}

function activateCheckboxesFromURL() {
  const urlPath = window.location.pathname;
  const filtersSegment = urlPath.split("/brand/")[1]?.split("/").pop() || "";
  ...
  document.querySelectorAll(".check-custom").forEach((checkbox) => {
    const transliterateOption = checkbox.getAttribute(
      "data-originalByBrand-transliterateOption",
    );
    ...
    if (filters.includes(transliterateOption)) {
      ...
      if (!square.classList.contains("active")) {
        ...
        activeFilters[filterType].add(
          checkbox.getAttribute("data-originalByBrand-option"),
        );
        const dropDownFilter = checkbox.closest(".drop-down-filter1");
...
function sortOptics(sortCriteria) {
  ...
  let filtersPath = baseUrl.split("/brand/")[1] || "";
  let newUrl = window.location.origin + `/brand/` + filtersPath;
